...
int rf[5000];
char* lexeme;
int halt, token, currInstruction = 0, programCounter = 0, symbolIndex = 0;
int ra = 0, bp = 0, sp = 0, level = 0,  regCount= 0, prevSymbolIndex;
...
void block()
{
	...
	// Allocate space on the stack for the AR
	sprintf(code[programCounter++], "6 0 0 4");

	// We don't really use return value
	sp += 1;
	// Store static link
	sprintf(code[programCounter++], "1 %d 0 %d", regCount, sp-1);
	sprintf(code[programCounter++], "4 %d 0 %d", regCount, sp++);
	// Store dynamic link
	sprintf(code[programCounter++], "1 %d 0 %d", regCount, sp-2);
	sprintf(code[programCounter++], "4 %d 0 %d", regCount, sp++);
	// Store return address
	// Need to edit this line of generated code whenever we encounter a Call with the correct return address
	sprintf(code[programCounter++], "1 %d 0 0", regCount);
	sprintf(code[programCounter++], "4 %d 0 %d", regCount, sp++);
	...
	if(token == constsym)
	{
		do
		{
			...
			symbolTable[symbolIndex].address = localCount+4;
	...
	while(token == procsym)
	{
		...
		// Save the address that we need to put a jump isntruction at
		int jumpInstruction = programCounter++;

		// Store the information about the procedure in the symbol table
		// The address is the program counter for where the call statment should jump to
		strcpy(symbolTable[symbolIndex].name, lexeme);
		symbolTable[symbolIndex].kind = 3;
		symbolTable[symbolIndex].address = programCounter;
		symbolTable[symbolIndex].level = level;
		symbolIndex++;
		...
		// Insert jump instruction before all code generated by block
		// Jumps the program past the procedure code
		sprintf(code[jumpInstruction], "7 0 0 %d", programCounter);
...
void statement()
{
	...
	if(token == identsym)
	{
		...
		if(target < 0)
			error(11);
		if(target < 0)
			printf("error in statement on: %s ", lexeme);

		// If not variable
		if(symbolTable[target].kind != 2)
			error(12);
		...
		sprintf(code[programCounter++], "4 %d %d %d", regCount-1, symbolTable[target].level, symbolTable[target].address);
	}
	else if(token == callsym)
	{
		...
		int procedureAddress = symbolTable[findIdentifier()].address;

		// We have to edit a line of generated code for this procedure to have to correct return address
		char newInstruction[20], buffer[5];
		// Copy all but the last char into newInstruction;
		strncpy(newInstruction, code[procedureAddress+3], strlen(code[procedureAddress+3])-1);
		// Put the return address in the buffer;
		itoa(programCounter+1, buffer, 10);
		// Put the return address in the newInstruction
		strcat(newInstruction, buffer);
		// replace the old instruction
		strcpy(code[procedureAddress+3], newInstruction);

		sprintf(code[programCounter++], "5 0 %d %d", level, procedureAddress);
	...
	else if(token == beginsym)
	{
		...
		// IF token != period sym then rtn?
		get();

		// If the next token isn't period, we just left a function call and need to return
		if(token != periodsym)
			sprintf(code[programCounter++], "2 0 0 0");
	}
	else if(token == ifsym)
	{
		int ifJump, elseJump, conditionReg;
		...
		// Put a jump instruction that either jumps to the next line of code, or over the else statement here
		elseJump = programCounter++;

		// We need to write in the jump instruction to skip the body of the if when the condition is false
		sprintf(code[ifJump], "8 %d 0 %d", conditionReg, programCounter);
		...
		// Fill in the empty space at elseJump in the code array
		sprintf(code[elseJump], "7 0 0 %d", programCounter);
	}
...
void factor()
{
	...
	if(token == identsym)
	{
		...
		if(value < 0)
			error(11);
		if(value < 0)
			printf("error in factor on : %s", lexeme);
...
void error(int errorCode)
{
	...
	printf("Error on %d", currInstruction);
	switch(errorCode)
	{
		case 1:
