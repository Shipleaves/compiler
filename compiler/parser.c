...
void statement()
{
	...
	if(token == identsym)
	{
		...
		sprintf(code[programCounter++], "4 %d %d %d", regCount-1, level - symbolTable[target].level, symbolTable[target].address);
	}
	else if(token == callsym)
	{
		...
		int procLevel = symbolTable[findIdentifier()].level;

		//sprintf(code[programCounter++], "5 0 %d %d", level, procedureAddress);
        sprintf(code[programCounter++], "5 0 %d %d", procLevel, procedureAddress);
...
// Forward search breaks on multiple variables with same name
/*int findIdentifier()
{
	int i;
